...
function initializeBooking() {
    ...
    const locationSelect = document.getElementById('location-preference');
    if (locationSelect) {
...
async function handleBookingSubmit(e) {
    ...
    const locationDisplay = {
        'home': `At home (${address})`,
        'remote': 'Remote over Zoom',
        'public': 'Local public place'
    }[location];
    ...
    formContainer.innerHTML = `
        <div class="booking-review-summary" style="background: #fff; padding: 20px; border: 2px solid var(--color-primary); border-radius: 8px; margin-bottom: 20px;">
            <h3 style="color: var(--color-primary); margin-bottom: 15px;">Review & Pay</h3>
    ...
    document.getElementById('cancel-review').onclick = () => {
        ...
        document.getElementById('location-preference').value = location;
        if (location === 'home') {
...
async function finalizeBooking(data) {
    const formContainer = document.getElementById('booking-form-container');
    formContainer.innerHTML = '<div style="text-align: center; padding: 40px;"><p>Finalizing your booking...</p></div>';
    ...
    try {
        ...
        // 3. Show confirmation
        showConfirmation(data.name, data.email, data.phone, data.notes, data.location, data.address);
    } catch (error) {
...
function generateGoogleCalendarUrl(name, notes) {
    ...
    url.searchParams.append('location', location === 'home' ? address : (location === 'remote' ? 'Zoom' : 'Rochester, NY'));

    return url.toString();
}

function generateCalendarEvent(name, email, phone, notes, location, address) {
    ...
    const icsContent = [
        ...
        `DESCRIPTION:${description}Service: ${selectedService.name}\\nLocation: ${locationDisplay}\\nDuration: ${selectedService.duration} minutes\\nPrice: $${selectedService.price} (Paid)\\n\\nCustomer: ${name}\\nEmail: ${email}\\nPhone: ${phone}\\n\\nSmartSass Tech\\n(585) 210-9758\\nsmartsasstech@gmail.com`,
        `LOCATION:${location === 'home' ? address : (location === 'remote' ? 'Zoom' : 'Rochester, NY')}`,
        'STATUS:CONFIRMED',
        'SEQUENCE:0',
        'BEGIN:VALARM',
        'TRIGGER:-PT1H',
...
function showConfirmation(name, email, phone, notes, location, address) {
    const container = document.querySelector('.booking-container');
    const googleUrl = generateGoogleCalendarUrl(name, notes, location, address);
    ...
    container.innerHTML = `
        ...
        day: 'numeric'
                ...
                <p><strong>Location:</strong> ${locationDisplay}</p>
                <p><strong>Duration:</strong> ${selectedService.duration} minutes</p>
                <p><strong>Price:</strong> $${selectedService.price} (Paid)</p>
            </div>
            ...
            <div class="calendar-options">
                ...
                <div class="calendar-btn-group">
                    ...
                    <button onclick="generateCalendarEvent('${name}', '${email}', '${phone}', '${notes}', '${location}', '${address}')" class="btn btn-apple">
                        Apple / Outlook
                    </button>
                </div>
            </div>
